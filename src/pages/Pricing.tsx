...
type Plan = {
  ...
  setup: string;
  featured: boolean;
  accentValues?: boolean;
  discountAccent?: boolean;
  setupAccent?: boolean;
  offsetClass?: string;
  withPattern?: boolean;
};

const bigTeamPlans: Plan[] = [
  {
    title: "1 месяц",
    price: "70 000 ₸",
    discount: "0%",
  ...
  return (
    <div className="mt-10 grid md:grid-cols-2 xl:grid-cols-4 gap-6">
      {plans.map((plan) => (
        <div
          key={plan.title}
          className={`relative overflow-hidden rounded-3xl border border-border/60 p-2 sm:p-3 shadow-sm ${
            plan.offsetClass ?? ""
          ...
          <div className={`relative rounded-2xl flex flex-col justify-between ${plan.featured ? "min-h-[620px]" : "min-h-[520px]"}`}>
            ...
            <div className="flex flex-col gap-3">
              ...
              <div className="rounded-2xl bg-[#C8D8EA] p-5">
                ...
                <p
                  className={`mt-3 text-xl leading-none font-semibold ${
                    plan.discountAccent ?? plan.accentValues ? "text-[#4A8DE8]" : "text-black"
                  }`}
                >
                  {plan.discount}
                </p>
              ...
              <div className="rounded-2xl bg-[#C8D8EA] p-5">
                ...
                <p
                  className={`mt-3 text-xl leading-none font-semibold ${
                    plan.setupAccent ?? plan.accentValues ? "text-[#4A8DE8]" : "text-black"
                  }`}
                >
                  {plan.setup}
                </p>
    ...
    <>
      ...
      <section className="py-16 lg:py-20 space-y-16">
        <Container>
          ...
          <PricingGrid plans={bigTeamPlans} />
        </Container>

        <Container>
          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
            <h2 className="text-4xl lg:text-4xl font-semibold leading-tight uppercase">
              Тарифные планы
              <br />
              для команды <span className="text-[#4A8DE8]">менее 10 человек</span>
            </h2>
          </div>

          <PricingGrid plans={smallTeamPlans} />
        </Container>
      </section>
      <Footer />
    </>
